...
const slides = [
  { image: '/slides/slide1.jpeg', comment: 'Gérez vos ventes en toute simplicité' },
  { image: '/slides/slide2.jpeg', comment: 'Une caisse rapide, intuitive et fiable' },
  { image: '/slides/slide3.jpeg', comment: 'Encaissez vos clients en quelques secondes' },
  { image: '/slides/slide4.jpeg', comment: 'Gardez une trace de chaque transaction' },
]
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'barkahflow-language' && e.newValue) {
        const index = langs.findIndex(l => l.code.toLowerCase() === e.newValue)
        if (index !== -1) {
          setLangIndex(index)
          initI18n(e.newValue)
        }
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="w-full md:w-[480px] flex flex-col relative bg-background">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-12 gap-6">
          ...
          {/* Logo */}
          <div className={`flex flex-col items-center gap-4 transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div
              ...
              <div className={`w-full transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                ...
                >
                  <span className="flex items-center justify-center gap-3">
                    ...
                    <span style={{ color: '#ffffff' }}>{t('login.google')}</span>
                  </span>
                </ShimmerButton>
              </div>
            </>
