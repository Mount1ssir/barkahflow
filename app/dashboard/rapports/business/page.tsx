...
export default function BusinessReportsPage() {
  ...
  const ledgerEntries = [
    { date: '2026-07-06', type: 'INFLOW', category: 'Sales Revenue', amount: 845000 }, // in centimes
    { date: '2026-07-05', type: 'OUTFLOW', category: 'Rent Payment', amount: 500000 },
    { date: '2026-07-04', type: 'INFLOW', category: 'Client Debt Settlement', amount: 250000 },
    { date: '2026-07-03', type: 'OUTFLOW', category: 'Utilities', amount: 85000 }
  ]
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full p-1 bg-slate-50/50 dark:bg-transparent min-h-screen">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">
              {t('reports.business.title', 'Business Reports')}
            </h1>
            <p className="text-sm text-slate-500 dark:text-zinc-400">
              {t('reports.business.subtitle', 'Analyse gross profit margins, operating costs, and overall ledger flows')}
            </p>
          </div>
        </div>

        <button 
          onClick={() => alert('Business Report Exported Successfully!')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-slate-500 dark:text-zinc-400 uppercase tracking-wider">
              {t('reports.business.profit_margin', 'Gross Profit Margin')}
            </span>
            ...
            <span className="text-xs text-emerald-600 dark:text-emerald-500 font-medium mt-1">
              +2.1% {t('reports.business.vs_last_q', 'vs last quarter')}
            </span>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-slate-500 dark:text-zinc-400 uppercase tracking-wider">
              {t('reports.business.total_expenses', 'Total Expenses')}
            </span>
            <span className="text-3xl font-bold text-rose-800 dark:text-rose-455 mt-1">
              {formatMAD(1845000)} {/* 18,450.00 MAD */}
            </span>
            <span className="text-xs text-slate-500 dark:text-zinc-400 mt-1">
              {t('reports.business.exp_count', 'Across 14 ledger bills')}
            </span>
          </div>
          <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400">
            <Receipt size={24} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 dark:border-zinc-800 flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-zinc-100">
            {t('reports.business.ledger_title', 'Cash Flow Ledger')}
          </h3>
        </div>
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-zinc-950/40 border-b border-slate-200 dark:border-zinc-800">
                ...
                <th className="px-6 py-3.5 text-xs font-bold text-slate-500 dark:text-zinc-400 tracking-wider">{t('reports.business.category', 'Category')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-slate-500 dark:text-zinc-400 tracking-wider text-right w-44">{t('reports.business.amount', 'Amount')}</th>
              </tr>
            </thead>
              ...
              {ledgerEntries.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50/30 dark:hover:bg-zinc-850/40 transition-colors duration-150">
                  ...
                  <td className={`px-6 py-4 text-xs font-bold text-right whitespace-nowrap ${
                    item.type === 'INFLOW' ? 'text-emerald-800 dark:text-emerald-450' : 'text-rose-800 dark:text-rose-455'
                  }`}>
