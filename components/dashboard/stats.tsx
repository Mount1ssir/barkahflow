...
interface StatCardProps {
  label: string
  value: string
  changePct?: number // undefined = pas de comparaison affichée (métrique snapshot)
...
function StatCard({ label, value, changePct, icon, iconBg, t }: StatCardProps) {
  const hasComparison = changePct !== undefined
    ...
    <Card className="rounded-2xl border shadow-sm hover:shadow-md transition-shadow duration-200">
      <CardContent className="p-5">
        ...
        {hasComparison ? (
          ...
          <div className="h-4" /> // maintient l'alignement des cartes sans comparaison
        )}
      </CardContent>
    </Card>
  )
...
export function Stats() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      />
      <StatCard
        label={t('dashboard.stats.sales', 'Ventes aujourd\'hui')}
        value={String(stats.todaySales)}
        changePct={stats.todaySalesChange}
